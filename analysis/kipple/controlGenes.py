...
def mapped(loci):
    ...
    mapped_loci = list()
    for locus in loci:
        if any([locus in ch for ch in chromosomes.values()]):
            mapped_loci.append(locus)
    return mapped_loci

def getControlGenes(loci):
    '''Get 5 loci either side of each locus'''
    if isinstance(loci, str): loci = [loci,]
    control_genes = list()
    for locus in loci:
        for Chr in chromosomes.values():
            if locus in Chr:
                ...
                control_genes.extend(Chr[slice(index-10, index + 11)])
